...
This paper does not evaluate whether the improved rank forecasts translate into portfolio-level returns (see Limitation~8). Preliminary gap-revision regressions (available in the replication archive) suggest that forecast gaps predict subsequent investment revisions, consistent with the forecasts capturing economically meaningful variation, but a full economic validation---including portfolio sorts and latency-adjusted backtests---remains for future work.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{No portfolio-level evaluation.} The paper reports forecast-accuracy metrics ($R^2$, IC, RMSE) but does not evaluate whether the improved rank forecasts translate into higher risk-adjusted portfolio returns. The mapping from a 1.2~percentile-point improvement in rank-forecast precision to portfolio performance depends on the cross-sectional return spread, transaction costs, and rebalancing frequency---quantities that are outside the scope of this methodological study. Until portfolio-level evidence is provided, the economic significance of the $\Delta R^2 = +0.047$ gain remains an open question.
\end{enumerate}
